...
clc
close all;
fid=load('.\compareScore\2010yushu7.1.txt');
...
ywYLim=[29.9 34.1];
ywDTLim=0;
ywDTXLim=[dtStart-1 dtEnd+1];
ywDTYLim=[2.5 8.5];

eqtFN='china3ml.eqt';
% eqtFN='tangshan.eqt';
[eqTime,eqLongi,eqLati,eqMag,eqDepth]=ywReadEQT(eqtFN);
m=find(eqTime>=dtStart & eqTime<=dtEnd & eqLongi>=leftUpLongi & eqLongi<=rightDownLongi & eqLati>=rightDownLati & eqLati<=leftUpLati & eqMag>=3);
...
% legend([{'judged should be judged'};{'wrong recognition'};{'unable to recognize'}]);
    ...
    ax.XAxis.TickLength=[0.03,0.03];
    ax.YAxis.TickLength=[0.03,0.03];
    xlabel('Longitude(бу)');ylabel('Latitude(бу)');
